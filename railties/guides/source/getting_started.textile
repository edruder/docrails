...
Most of the work in this tutorial will happen in the <tt>app/</tt> directory, but here's a basic rundown on the function of each directory that Rails creates in a new application by default:
...
|app/views/posts/_form.html.erb               |A partial for the form shared by the edit and new views|
...
The +create+ action instantiates a new Post object from the data supplied by the user on the form, which Rails makes available in the +params+ hash. After successfully saving the new post, the +create+ action returns the appropriate format that the user requested (HTML in our case), then redirects the user to the resulting post +show+ action and sets a notice to the user that the post was successfully created.

If the post was not successfully saved, perhaps due to a validation error, then the controller returns the user back to the +new+ action with any error messages so that the user has the chance to fix the error and try again.

The "Post was successfully created" message is stored inside of the Rails +flash+ hash (usually just called the Flash), so that messages can be carried over to the next action, providing the user with useful information on the status of their request. In the case of +create+, the user never actually sees any page rendered during the post creation process, because it immediately redirects to the new post as soon Rails saves the record. The Flash carries over a message to the next action, so that when the user is redirected to the +show+ action, they are presented with a message saying "Post was successfully created."
...
Like creating a new post, editing a post is a two-part process. The first step is a request to a URL like +http://localhost:3000/posts/1/edit+ (the +edit_post_path+ helper method generates this URL, given a particular post). This calls the +edit+ action in the controller:
...
Models in Rails use a singular name, and their corresponding database tables use a plural name. A reasonable name for the model handling comments would be Comment, with a corresponding database table named comments. Even if you don't want to use the entire apparatus set up by scaffolding, most Rails developers still use generators to make things like models and controllers. To create the new model, run this command in your terminal:
...
As with the +home+ controller, we will need to add a route so that Rails knows where we would like to navigate to see +comments+.  Open up the +config/routes.rb+ file again and navigate to the entry that was added automatically for +posts+ near the top by the scaffold generator, +resources :posts+. Edit it as follows:
...
As with any blog, our readers will create their comments directly after reading the post, and once they have added their comment, will be sent back to the post +show+ page with their new comment listed.  Our +CommentsController+ is there to provide a method to create comments and to delete spam comments when they arrive.

First, we'll wire up the Post +show+ template (+/app/views/posts/show.html.erb+) to let us make a new comment:
...
You'll see a bit more complexity here than you did in the controller for posts. That's a side-effect of the nesting that you've set up -- each request for a comment has to keep track of the post to which the comment is attached, thus the initial call to the Post model's +find+ method to get the post in question.

In addition, the code takes advantage of some of the methods available for a +has_many+ association -- the Post model's +comments+ method returns the collection of comments that are associated with a particular post and calling +create+ on that collection makes a new comment associated with the post and saves it to the database.

Once we have made the new comment, we send the user back to the +post_path(@post)+ URL.  This runs the +show+ action of the +PostsController+ which then renders the +show.html.erb+ template where we want the comment to show. Let's add that to the +app/view/posts/show.html.erb+:
...
Now that we have Posts and Comments working, let's take a look at the +app/views/posts/show.html.erb+ template--it's getting a bit long and awkward. We can use partials to clean it up.

h4. Rendering Partial Collections

First we'll make a comment partial to extract showing all the comments for the post. Create a file +app/views/comments/_comment.html.erb+ and put into it:
...
Then, change +app/views/posts/show.html.erb+ to look like the following:
...
Let's also move that new comment section out to its own partial. Again, you create a file +app/views/comments/_form.html.erb+ and in it you put:
...
Then you make +app/views/posts/show.html.erb+ look like the following:
...
The second render just specifies the partial template we want to render, <tt>comments/form</tt>. Rails is smart enough to spot the forward slash in that string and realize that you want to render the <tt>_form.html.erb</tt> partial in the <tt>app/views/comments</tt> directory.
